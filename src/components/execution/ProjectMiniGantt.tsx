...
/**
 * Kompaktní read-only Gantt projektu (project_milestones). Používá se
 * v přiřazení subdodavatele a počítá se s ním i pro portál subdodavatele
 * (fáze 2) — subka si termíny realizace najde sama.
 */
export default function ProjectMiniGantt({ projectId, onPickRange }: Props) {
  ...
  const pos = (t: number) => ((t - minT) / span) * 100;
  const todayT = Date.now();
  const todayPct = todayT >= minT && todayT <= maxT ? pos(todayT) : null;

  // popisky měsíců na časové ose
  ...
  while (cursor.getTime() < maxT) {
    ...
    if (t >= minT) {
      monthTicks.push({ label: cursor.toLocaleDateString('cs-CZ', { month: 'short', year: '2-digit' }), pct: pos(t) });
    }
  ...
  return (
    <div className="bg-white/[0.04] border border-white/[0.08] rounded-xl p-3 space-y-1.5">
      ...
      <div className="relative h-4 border-b border-white/[0.06]">
        {monthTicks.map((tick, i) => (
          <span key={i} className="absolute top-0 text-[9px] text-slate-500 -translate-x-1/2" style={{ left: `${tick.pct}%` }}>
            {tick.label}
          </span>
        ))}
      </div>
      ...
      <div className="relative space-y-1 pt-1">
        {todayPct !== null && (
          <div className="absolute top-0 bottom-0 w-px bg-red-400/70 z-10" style={{ left: `${todayPct}%` }} title="Dnes" />
        )}
        {milestones.map(m => {
          const left = pos(new Date(m.start_date).getTime());
          const width = Math.max(pos(new Date(m.end_date).getTime() + DAY) - left, 1.5);
          ...
          return (
            <div key={m.id} className="relative h-6">
              <button
                ...
                title={`${m.name} · ${range}`}
                className={`absolute top-0 h-6 rounded-md overflow-hidden text-left border border-black/20 ${onPickRange ? 'cursor-pointer hover:ring-2 hover:ring-white/40' : 'cursor-default'}`}
                style={{ left: `${left}%`, width: `${width}%`, backgroundColor: `${m.color || '#3b82f6'}55` }}
              >
                <div className="absolute inset-y-0 left-0" style={{ width: `${m.progress || 0}%`, backgroundColor: m.color || '#3b82f6' }} />
                <span className="relative z-10 px-1.5 text-[10px] font-bold text-white whitespace-nowrap leading-6 drop-shadow">
                  {m.name}
                </span>
              </button>
            </div>
